  ...
  <nav class="bg-white shadow-sm py-4 px-6 md:px-12 flex justify-between items-center sticky top-0 z-50 rounded-b-xl">
    <div class="flex items-center space-x-2">
      <svg class="w-8 h-8 text-blue-600" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
        <path
          d="M10 2a8 8 0 100 16 8 8 0 000-16zm-3 7a1 1 0 011-1h4a1 1 0 110 2H8a1 1 0 01-1-1zm1 3a1 1 0 000 2h4a1 1 0 100-2H8z">
        </path>
      </svg>
    ...
    <div class="space-x-6 hidden md:flex">
      <NuxtLink to="/"
        class="nav-link text-gray-600 hover:text-blue-600 transition-colors duration-200 py-2 px-1 rounded-md">Início
      </NuxtLink>

      <!-- Aluno - visível para admin e aluno -->
      <NuxtLink v-if="user?.email === 'admin@admin.com' || user?.email === 'aluno@aluno.com'" to="/student"
      ...
      <NuxtLink v-if="user?.email === 'admin@admin.com' || user?.email === 'professor@professor.com'" to="/teacher"
        class="nav-link text-gray-600 hover:text-blue-600 transition-colors duration-200 py-2 px-1 rounded-md">
        Professor
      </NuxtLink>

      <NuxtLink v-if="user" to="/resources"
        class="nav-link text-gray-600 hover:text-blue-600 transition-colors duration-200 py-2 px-1 rounded-md">
        Recursos
      </NuxtLink>

      <NuxtLink v-if="user" to="/forum"
        class="nav-link text-gray-600 hover:text-blue-600 transition-colors duration-200 py-2 px-1 rounded-md">
        Comunicação
      </NuxtLink>

      <NuxtLink v-if="user?.email === 'admin@admin.com'" to="/admin"
        class="nav-link text-gray-600 hover:text-blue-600 transition-colors duration-200 py-2 px-1 rounded-md">
