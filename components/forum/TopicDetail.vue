<template>
        ...
        <div class="flex justify-between items-start mb-4">
          <div class="flex items-center">
            ...
            <div>
              ...
              <p class="text-sm text-gray-500">{{ topic.postedAt }} • <span :class="levelColor">{{ topic.author.level }}</span></p>
            </div>
          </div>
  ...
  const levelColor = computed(() => {
    switch(props.topic.author.level) {
      case 'Iniciante': return 'text-blue-600'
      case 'Intermediário': return 'text-green-600'
      case 'Especialista': return 'text-purple-600'
  ...
  const vote = (type) => {
    if (type === 'up') {
      props.topic.votes.up++
    } else {
      props.topic.votes.down++
    }
  }
  ...
  const postReply = () => {
    ...
    if (content.trim()) {
      props.topic.replies.push({
        id: props.topic.replies.length + 1,
        content,
        author: {
          name: 'Você',
